...
def update_booking_status(booking_id):
    ...
    if not booking: return {"error": "Not found"}, 404

    if new_status == "confirmed":
        learner_id, teacher_id = booking["learnerId"], booking["teacherId"]
        learner = users_col.find_one({"_id": ObjectId(learner_id)})
        if learner.get("creditBalance", 0) < 1: return {"error": "No credits"}, 400
        ...
        db.transactions.insert_one({"learnerId": learner_id, "teacherId": teacher_id, "amount": 1, "timestamp": datetime.now(timezone.utc), "type": "skill_swap"})
...
def social_login():
    ...
    if not user:
        new_user = {
            ...
            "passwordHash": "SOCIAL_AUTH_NO_PASSWORD", "creditBalance": 10, "roles": ["learner"],
            "skillTags": [], "learningSkills": [], "createdAt": datetime.now(timezone.utc)
        }
        result = users_col.insert_one(new_user)
        user_id = str(result.inserted_id)
    else: user_id = str(user["_id"])
    return jsonify({"status": "success", "userId": user_id, "name": data.get("name")})
